...
class CFARCoreWithMem[T <: Data : Real : BinaryRepresentation](val params: CFARParams[T]) extends Module {
  ...
  val retimingInt = if (params.retiming) 1 else 0
  ...
  // val lastOut = ShiftRegister(cellUnderTest.io.lastOut, thresholdPip + retimingInt, io.out.fire())
  val lastOut = ShiftRegister(cellUnderTest.io.lastOut, thresholdPip + retimingInt, true.B)
  ...
  when (cntIn === (latency - 1.U) && io.in.fire()) { //&& io.in.fire()) {
  ...
  //val cutDelayed = ShiftRegister(cellUnderTest.io.out.bits, depthOfQueue, en = cellUnderTest.io.out.fire() || (flushing && io.out.ready))
  val readyPipe = if (depthOfQueue != 0) Some(WireInit(false.B)) else None
  ...
  //en = io.in.fire() || (flushing && readyPipe.get)) else cellUnderTest.io.out.bits
  ...
  if (params.numAddPipes == 0 && params.numMulPipes == 0) {
    ...
    queueData.io.enq.valid := ShiftRegister(initialInDone, depthOfQueue, en = true.B) && io.in.fire() || (flushing && ShiftRegister(readyPipe.get, depthOfQueue, en = true.B))
    if (params.sendCut)
    ...
    readyPipe.get := queueData.io.enq.ready
    queueFftBin.io.enq.valid := ShiftRegister(initialInDone, depthOfQueue, en = true.B) && io.in.fire() || (flushing && ShiftRegister(readyPipe.get, depthOfQueue, en = true.B))
    ...
    queueLast.io.enq.valid := ShiftRegister(initialInDone, depthOfQueue, en = true.B) && io.in.fire() || (flushing && ShiftRegister(readyPipe.get, depthOfQueue, en = true.B))
