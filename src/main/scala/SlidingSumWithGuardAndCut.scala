...
class SlidingSumWithGuardAndCut[T <: Data : Real : BinaryRepresentation](val params: SlidingSumParams[T]) extends Module {
  ...
  val latency = io.depthRunTime.getOrElse(params.depth.U) +& (io.guardRunTime.getOrElse(params.guardCells.U) >> 2.U) +& 1.U

  assert(io.windowRunTime.getOrElse(params.testWindowSize.U) > 2.U * io.depthRunTime.getOrElse(params.depth.U) + io.guardRunTime.getOrElse(params.guardCells.U) + 1.U, "Windowing size shall be larger than total number of shifting cells inside sliding sum")
  assert(io.guardRunTime.getOrElse(params.guardCells.U) > 0.U, "Number of guard cells should be greater than 0")

  val sumT: T = (io.in.bits * log2Ceil(params.depth)).cloneType // no overflow
  val sumlagg = RegInit(t = sumT, init = 0.U.asTypeOf(sumT))
  val sumlead = RegInit(t = sumT, init = 0.U.asTypeOf(sumT))

  val slidingWin1 = Module(new AdjustableShiftRegisterStream(params.protoIn, params.depth))
  slidingWin1.io.in <> io.in
  slidingWin1.io.depth := io.depthRunTime.getOrElse(params.depth.U)
  slidingWin1.io.lastIn := io.lastIn

  val lastOut = ShiftRegister(slidingWin1.io.lastOut, params.guardCells/2, true.B) // treba da bude adjustable shift register, gde mu je depth run-time konfigurabilan
  val guardCells = Module(new AdjustableShiftRegisterStream(params.protoIn, params.guardCells))
  ...
  val slidingWin2 = Module(new AdjustableShiftRegisterStream(params.protoIn, params.depth))
  ...
  slidingWin1.io.out.ready := guardCells.io.in.ready // this is a question, maybe it will not be bidirectional connection
